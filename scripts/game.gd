...
func _process(delta: float) -> void:
	...
	player_2_inactivity_timer += delta
	
	# Si les deux joueurs sont inactifs pendant la durée limite, arrêter le jeu
	if player_1_inactivity_timer >= inactivity_duration and player_2_inactivity_timer >= inactivity_duration:
	...
	var player_2_x = player_2.position.x
	
	# Déterminer quel joueur est le plus à droite
	var target_position_x = max(player_1_x, player_2_x)
	...
	# Vérifier si Player2 sort des limites de la caméra
	if player_2.position.x < camera_left_limit or player_2.position.x > camera_right_limit:
	...
	if player_2.position.y > ground_limit_y:
...
# Fonction appelée pour réinitialiser le timer d'inactivité du joueur 2
func reset_player_2_inactivity_timer() -> void:
	player_2_inactivity_timer = 0.0
