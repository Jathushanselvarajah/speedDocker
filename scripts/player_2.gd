...
func _physics_process(delta: float) -> void:
	if not game_timer.race_active or is_frozen:
		return  # Sort de la fonction si le joueur est bloqué
	# Ajout de la gravité.
	if not is_on_floor():
	...
	if Input.is_action_just_pressed("player2_jump") and is_on_floor():
		...
		game_node.reset_player_1_inactivity_timer()  # Réinitialiser le timer du joueur 2
	...
	if direction != 0:  # Si le joueur 2 se déplace, réinitialiser aussi le timer
		game_node.reset_player_2_inactivity_timer()
