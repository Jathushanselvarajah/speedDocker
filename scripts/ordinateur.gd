...
func _on_ordinateur_body_entered(body: Node2D) -> void:
	if body.is_in_group("Players"):  # Vérifie que le corps est un joueur
		print("Collision avec un joueur détectée : " + body.name)  # Affiche quel joueur a touché l'ordinateur
		...
		if body.name == "Player1":
			...
			print("Player 1 a pris l'ordinateur, Player 2 est bloqué pour 0.3s")  # Affiche dans la console
		# Si c'est le joueur 2 qui touche l'ordinateur
		elif body.name == "Player2":
			player1.freeze(0.3)  # Applique l'effet sur le joueur 1 pendant 0,3 seconde
			print("Player 2 a pris l'ordinateur, Player 1 est bloqué pour 0.3s")  # Affiche dans la console
